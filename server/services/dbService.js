...
if (isSupabaseEnabled) {
  console.log('[Database] Inicializando cliente Supabase...');
  supabase = createClient(supabaseUrl, supabaseKey);
} else {
  console.log('[Database] Utilizando banco de dados local SQLite.');
}
...
export const dbService = {
  ...
  leads: {
    ...
    getStats: async () => {
      if (isSupabaseEnabled) {
        // Obter contadores do Supabase
        const { count: totalLeads } = await supabase.from('leads').select('*', { count: 'exact', head: true });
        
        // Hoje (formato ISO YYYY-MM-DD)
    ...
    // 4. Buscar lead existente para evitar duplicação
      ...
      if (isSupabaseEnabled) {
        ...
        const result = await dbRun(
          `INSERT INTO leads (
            ...
            proposal_text, proposal_sent, created_at, updated_at
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [
    ...
    updateLeadCrm: async (id, crmData) => {
      ...
      if (isSupabaseEnabled) {
        ...
        await dbRun(
          `UPDATE leads SET owner = ?, value_negotiated = ?, next_action = ?, notes = ?, status = ?, first_contact_date = ?, last_contact_date = ?, history = ?, proposal_text = ?, proposal_sent = ?, updated_at = ? WHERE id = ?`,
          [
            crmData.owner,
  ...
  followUps: {
    ...
    getPendingFollowUps: async (nowTime) => {
      if (isSupabaseEnabled) {
        ...
        // Formatar no mesmo modelo do SQLite JOIN
        return (data || []).map(f => ({
          id: f.id,
          lead_id: f.lead_id,
    ...
    // 2. Atualizar status de envio de follow-up
    ...
    // 3. Deletar follow-ups agendados (ao re-agendar)
  ...
  settings: {
    // 1. Obter todas as configurações como objeto Key-Value
    getSettings: async () => {
      if (isSupabaseEnabled) {
